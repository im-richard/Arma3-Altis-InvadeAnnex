...
if (count _this == 1) then
	...
	if (_marcador isEqualType "") then
		...
		{
		if (typeOf _marcador == "I_Soldier_SL_F") then {_chance = 20}
		else
			{
			if ((typeOf _marcador == "I_Soldier_TL_F") or (typeOf _marcador == "I_helipilot_F") or (typeOf _marcador == "I_pilot_F")) then {_chance = 15}
			else
				{
				if (typeOf _marcador == "I_officer_F") then {_chance = 50};
				};
			};
		};
	};
...
if (count _minasAAF > 0) then
	{
	{if (random 100 < _chance) then {west revealMine _x; _revelaMina = true}} forEach _minasAAF;
	};
