...
[] call playerIsOwner;
_tsk = ["DES",true,[format ["We have downed air vehicle. It is a good chance to destroy it before it is recovered. Do it before a recovery team from the %1 reaches the place. MOVE QUICKLY",_nombrebase],"Destroy Air",_mrkfin],_posCrashMrk,"CREATED",5,true,true,"Destroy"] call BIS_fnc_setTask;
_vehiculos = [];
_soldados = [];
_grupos = [];
...
_grpcrash = createGroup independent;
...
_vehicle=[position _road, 0,"I_MRAP_03_F", independent] call bis_fnc_spawnvehicle;
...
{_x assignAsCargo _veh; _x moveInCargo _veh; _soldados = _soldados + [_x]; [_x] join _grupoveh; [_x] spawn AAFinit} forEach units _grupo;
deleteGroup _grupo;
...
_vehicleT=[position _road, 0,"I_Truck_02_transport_F", independent] call bis_fnc_spawnvehicle;
_vehT = _vehicleT select 0;
[_vehT] spawn AAFVEHinit;
[_vehT,"AAF Recover Truck"] spawn inmuneConvoy;
_vehCrewT = _vehicle select 1;
{[_x] spawn AAFinit} forEach _vehCrewT;
...
if (not alive _heli) then
	{
	[] call playerIsOwner;
	_tsk = ["DES",true,[format ["We have downed air vehicle. It is a good chance to destroy it before it is recovered. Do it before a recovery team from the %1 reaches the place. MOVE QUICKLY",_nombrebase],"Destroy Air",_mrkfin],_posCrashMrk,"SUCCEEDED",5,true,true,"Destroy"] call BIS_fnc_setTask;
	[0,300] remoteExecCall ["resourcesFIA",2];
	[5,0] remoteExecCall ["prestige",2];
	[-3,3,_posicion] remoteExecCall ["citySupportChange",2];
...
if ((dateToNumber date > _fechalimnum) or (_vehT distance _posicion < 100)) then
	{
	[] call playerIsOwner;
	_tsk = ["DES",true,[format ["We have downed air vehicle. It is a good chance to destroy it before it is recovered. Do it before a recovery team from the %1 reaches the place. MOVE QUICKLY",_nombrebase],"Destroy Air",_mrkfin],_posCrashMrk,"FAILED",5,true,true,"Destroy"] call BIS_fnc_setTask;
	[3,0,_posicion] remoteExecCall ["citySupportChange",2];
