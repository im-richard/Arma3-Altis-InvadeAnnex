...
[] call playerIsOwner;
_tsk = ["REP",true,[format ["AAF is rebuilding a radio tower in %1. If we want to keep up the enemy comms breakdown, the work must be stopped. Destroy the repair truck parked nearby or capture the zone. Work will be finished on %2:%3.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Tower Rebuild Disrupt",_marcador],_posicion,"CREATED",5,true,true,"Destroy"] call BIS_fnc_setTask;
_camionCreado = false;
...
if (spawner getVariable _marcador) then
	...
	_pos = [];
	_pos = _posicion findEmptyPosition [10,60,"I_Truck_02_box_F"];
	_veh = createVehicle ["I_Truck_02_box_F", _pos, [], 0, "NONE"];
	_veh allowdamage false;
	_veh setDir random 360;
	_nul = [_veh] spawn AAFVEHinit;
	player setpos position _veh;
