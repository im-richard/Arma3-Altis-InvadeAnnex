...
if (_tipo == "delete") exitWith
	...
	[_hr,_coste] remoteExecCall ["resourcesFIA",2];
	deleteMarker _mrk;
...
[] call playerIsOwner;
_tsk = ["PuestosFIA",true,["We are sending a team to establish an Observation Post or Roadblock. Send and cover the team until reaches it's destination.","Post \ Roadblock Deploy",_mrk],_posicionTel,"CREATED",5,true,true,"Move"] call BIS_fnc_setTask;

_grupo = [getMarkerPos "respawn_west", WEST, (configfile >> "CfgGroups" >> "West" >> "Guerilla" >> "Infantry" >> _tipogrupo)] call BIS_Fnc_spawnGroup;
...
waitUntil {sleep 1; ({alive _x} count units _grupo == 0) or ({(alive _x) and (_x distance _posicionTel < 10)} count units _grupo > 0)};
...
if ({(alive _x) and (_x distance _posicionTel < 10)} count units _grupo > 0) then
	...
	[] call playerIsOwner;
	_tsk = ["PuestosFIA",true,["We are sending a team to establish an Observation Post or Roadblock. Send and cover the team until reaches it's destination.","Post \ Roadblock Deploy",_mrk],_posicionTel,"SUCCEEDED",5,true,true,"Move"] call BIS_fnc_setTask;
	_nul = [-5,5,_posiciontel] remoteExecCall ["citySupportChange",2];
	_mrk setMarkerType "loc_bunker";
	_mrk setMarkerColor "ColorBlue";
	_mrk setMarkerText _texto;
	}
	...
	{
	[] call playerIsOwner;
	_tsk = ["PuestosFIA",true,["We are sending a team to establish an Observation Post or Roadblock. Send and cover the team until reaches it's destination.","Post \ Roadblock Deploy",_mrk],_posicionTel,"FAILED",5,true,true,"Move"] call BIS_fnc_setTask;
	sleep 3;
	deleteMarker _mrk;
	};
