...
if (!isNull _jugador) then
	...
	_arevelar = [];
	{
	if (((side _x == independent) or (side _x == EAST)) and (alive _x) and (not(fleeing _x)) and (_x distance _posicion < _size)) then {_arevelar pushBack _x};
	} forEach allUnits;
...
if (_marcador in aeropuertos) then
	{
	[0,10,_posicion] remoteExecCall ["citySupportChange",2];
	[["TaskSucceeded", ["", "Airport Taken"]],"BIS_fnc_showNotification"] call BIS_fnc_MP;
	_nul = [20,10] remoteExecCall ["prestige",2];
...
if (_marcador in bases) then
	...
	_minasAAF = allmines - (detectedMines west);
	if (count _minasAAF > 0) then
		{
		{if (_x distance _pos < 1000) then {west revealMine _x}} forEach _minasAAF;
		};
	};
...
if (_marcador in puertos) then
	{
	//_nul = [-10,0,_posicion] remoteExecCall ["citySupportChange",2];
	[["TaskSucceeded", ["", "Seaport Taken"]],"BIS_fnc_showNotification"] call BIS_fnc_MP;
	_nul = [10,10] remoteExecCall ["prestige",2];
...
if ((_marcador in fabricas) or (_marcador in recursos)) then
	...
	if (_powerpl in mrkAAF) then
		{
		server SetVariable [_marcador,false];
		sleep 5;
		...
		{
		server SetVariable [_marcador,true];
