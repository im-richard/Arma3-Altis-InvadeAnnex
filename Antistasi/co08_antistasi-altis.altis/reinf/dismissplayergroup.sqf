...
player sideChat "Get out of my sight you useless scum!";

_ai = false;

if ({isPlayer _x} count units group player == 1) then {_ai = true; _nuevogrp = createGroup WEST};
...
if (_ai) then
	...
	if (!isMultiplayer) then {_nul = [_hr,_resourcesFIA] remoteExecCall ["resourcesFIA",2];} else {_nul = [_hr,0] remoteExecCall ["resourcesFIA",2]; [_resourcesFIA] call resourcesPlayer};
