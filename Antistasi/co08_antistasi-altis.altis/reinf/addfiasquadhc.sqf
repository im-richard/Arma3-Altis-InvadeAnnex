...
_chequeo = false;
{
	if (((side _x == east) or (side _x == independent)) and (_x distance petros < 500) and (not(captive _x))) then {_chequeo = true};
} forEach allUnits;
...
_nul = [- _costeHR, - _coste] remoteExecCall ["resourcesFIA",2];
...
if (_esinf) then
	...
	_pos = [position petros, 30, random 360] call BIS_Fnc_relPos;
	_grupo = [_pos, WEST, (configfile >> "CfgGroups" >> "West" >> "Guerilla" >> "Infantry" >> _tipogrupo)] call BIS_Fnc_spawnGroup;
	...
	_formato = format ["%1%2",_formato,{side (leader _x) == west} count allGroups];
	...
	_vehicle=[_pos, 0,"B_G_Van_01_transport_F", west] call bis_fnc_spawnvehicle;
	...
	if (_tipogrupo == "B_G_Mortar_01_F") then
		...
		_grupo setGroupId [format ["Mort-%1",{side (leader _x) == west} count allGroups]];
		...
		_mortero setDir (getDir _camion + 180);
		_morty moveInGunner _mortero;
		if (_tipogrupo == "B_static_AT_F") then {_grupo setGroupId [format ["M.AT-%1",{side (leader _x) == west} count allGroups]]};
		if (_tipogrupo == "B_static_AA_F") then {_grupo setGroupId [format ["M.AA-%1",{side (leader _x) == west} count allGroups]]};
		};
...
_nul = [0, - _coste] remoteExecCall ["resourcesFIA",2];
