...
if (_tipo == "rem") then
	...
	if (spawner getVariable _cercano) then
		{
		if ({(alive _x) and (!captive _x) and ((side _x == independent) or (side _x == EAST)) and (_x distance _posicion < 500)} count allUnits > 0) then
			{
			...
			{
			if ((side _x == west) and (not(_x getVariable ["BLUFORSpawn",false])) and (_x distance _posicion < _size) and (_x != petros)) then
				{
				if (!alive _x) then
					{
	...
	if (spawner getVariable _cercano) then
		{
		if ({(alive _x) and (!captive _x) and ((side _x == independent) or (side _x == EAST)) and (_x distance _posicion < 500)} count allUnits > 0) exitWith {hint "You cannot add soldiers to this garrison while there are enemies nearby"; _nul=CreateDialog "garrison_menu"};
		};
