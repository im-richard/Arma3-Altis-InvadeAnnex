...
[] call playerIsOwner;
_tsk = ["NATOArty",true,[format ["We have NATO Artillery support from %1. They will be under our command until %2:%3.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"NATO Arty",_marcador],_posicion,"CREATED",5,true,true,"Support"] call BIS_fnc_setTask;
...
if ({alive _x} count _vehiculos == 0) then
	...
	[] call playerIsOwner;
	_tsk = ["NATOArty",true,[format ["We have NATO Artillery support from %1. They will be under our command until %2:%3.",_nombredest,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"NATO Arty",_marcador],_posicion,"FAILED",5,true,true,"Support"] call BIS_fnc_setTask;
	};
