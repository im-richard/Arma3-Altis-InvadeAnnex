...
if (debug) then {stavros SideChat format ["AutoGarrison en marcha, destino %1",_marcador]};
...
while {(_size > 0)} do
	...
	if (diag_fps > minimoFPS) then
		...
		_vehicle=[_pos, random 360,_tipoVeh, WEST] call bis_fnc_spawnvehicle;
		...
		if (_tipoVeh != "B_G_Offroad_01_armed_F") then
			...
			if (_tipoVeh == "B_G_Quadbike_01_F") then
				...
				_grupo = [_origen, WEST, (configfile >> "CfgGroups" >> "West" >> "Guerilla" >> "Infantry" >> _tipoGrupo)] call BIS_Fnc_spawnGroup;
