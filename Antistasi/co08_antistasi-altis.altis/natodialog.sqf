...
_exists = [_tipo] call BIS_fnc_taskExists;
if (_exists) exitWith {hint "NATO is already busy with this kind of mission"};

_bases = bases - mrkAAF;

if (((_tipo == "NATOArty") or (_tipo == "NATOArmor")) and (count _bases == 0)) exitWith {hint "You need to conquer at least one base to perform this action"};
