...
_busy = server getVariable _base;
if ((_pos distance _posbase < 7500) and (_pos distance _posbase > 2500) and (not (spawner getVariable _base)) and (not _busy)) then {_bases = _bases + [_base]}
} forEach _basesAAF;
