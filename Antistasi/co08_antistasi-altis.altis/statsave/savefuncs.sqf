...
fn_SetStat =
{
	...
	if(_varName in specialVarLoads) then
	{
		...
		if(_varName == 'rankPlayer') then {player setUnitRank _varValue};
		...
		*/
		...
		if(_varName == 'planesAAFcurrent') then
			...
			if ((planesAAFcurrent > 0) and (count planesAAF < 2)) then {planesAAF = planesAAF + ["I_Plane_Fighter_03_CAS_F","I_Plane_Fighter_03_AA_F"]; publicVariable "planesAAF"}
			};
		if(_varName == 'helisAAFcurrent') then
			...
			if ((helisAAFcurrent > 0) and (not ("I_Heli_light_03_F" in planesAAF))) then {planesAAF = planesAAF + ["I_Heli_light_03_F"]; publicVariable "planesAAF"};
			};
		if(_varName == 'APCAAFcurrent') then
			...
			if ((APCAAFcurrent > 0) and (not ("I_APC_Wheeled_03_cannon_F" in vehAAFAT))) then {vehAAFAT = vehAAFAT +  ["I_APC_Wheeled_03_cannon_F","I_APC_tracked_03_cannon_F"]; publicVariable "vehAAFAT"};
			};
		if(_varName == 'tanksAAFcurrent') then
			...
			if ((tanksAAFcurrent > 0) and (not ("I_MBT_03_cannon_F" in vehAAFAT))) then {vehAAFAT = vehAAFAT +  ["I_MBT_03_cannon_F"]; publicVariable "vehAAFAT"};
			};
		...
		if(_varName == 'minas') then
			...
			for "_i" from 0 to (count _varvalue) - 1 do
				...
				*/
				...
				if (_detectada) then {west revealMine _mina};
		...
		if(_varname == 'prestigeOPFOR') then
			...
			for "_i" from 0 to (count ciudades) - 1 do
				...
				//_datos = [_numCiv,_numVeh,_roads,_prestigeOPFOR,_prestigeBLUFOR,_powered];
		...
		if(_varName == 'posHQ') then
			...
			if (isMultiplayer) then
				{
				sleep 5;
				{_x setPos position Petros} forEach playableUnits;
				}
			else
				{
