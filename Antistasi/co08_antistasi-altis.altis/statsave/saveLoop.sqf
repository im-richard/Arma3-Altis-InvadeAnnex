if (!isDedicated) then
	...
	if (isMultiplayer) then
		...
		{if ((!isPlayer _x) and (not(_x getVariable "inconsciente"))) then {_resfondo = _resfondo + (server getVariable (typeOf _x))}
...
_hrfondo = (server getVariable "hr") + ({(alive _x) and (not isPlayer _x) and (not(_x getVariable "inconsciente")) and (_x getVariable ["BLUFORSpawn",false])} count allUnits);
...
if (_amigo getVariable ["BLUFORSpawn",false]) then
	...
	if ((alive _amigo) and (!isPlayer _amigo)) then
		{
		if ((isPlayer leader _amigo) or (group _amigo in (hcAllGroups stavros))) then
			{
...
_dirMina = getDir _x;
_detectada = false;
if (_x mineDetectedBy west) then {_detectada = true};
