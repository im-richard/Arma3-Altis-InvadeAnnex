...
if (vehicle _unit != _unit) then
	...
	if (_isPlayer) then
		{
		{
		if ((!isPlayer _x) and (vehicle _x != _x) and (_x distance _unit < 50)) then {unassignVehicle _x}
...
while {(time < _bleedOut) and (damage _unit > 0.25) and (alive _unit) and (_unit getVariable "inconsciente") and (!(_unit getVariable "respawning"))} do
	...
	if (_isPlayer) then
		...
		if (isNil "_ayudado") then
			...
			if (!isNil "_ayuda") then {_camTarget = _ayuda};
			};
...
if (_isPlayer) then
	...
	//if (isMultiplayer) then {deleteMarker _mrk};
	}
else
	{
	_unit stop false;
...
if (time > _bleedOut) exitWith
	...
	if (_isPlayer) then
		...
		_ayudado = _unit getVariable "ayudado";
		if (!isNil "_ayudado") then {_unit setdamage 0.2} else {[_unit] call respawn};
		}
	else
		{
		_unit setDamage 1;
