...
_exists = ["PuestosFIA"] call BIS_fnc_taskExists;
if (_exists) exitWith {hint "We can only deploy / delete one Post or Roadblock at a time."};
...
if ((_tipo == "delete") and ({(alive _x) and (!captive _x) and ((side _x == independent) or (side _x == EAST)) and (_x distance _posicionTel < 500)} count allUnits > 0)) exitWith {hint "You cannot delete a Post while enemies are near it"};
