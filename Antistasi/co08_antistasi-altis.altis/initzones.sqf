...
if (_x in aeropuertos) then
    ...
    server setVariable [_x,false,true];//el false es para verificar que si han mandado tropas de ataque, lo definiremos con un _busy
    _nul = [_x] call crearControles;
    garrison setVariable [_x,[],true];
    };
if (_x in bases) then
...
if (_x in recursos) then
    ...
    server setVariable [_x,true,true];//el penultimo true es para ver si tienen electricidad,lo definiremos con un _powered
    _nul = [_x] call crearControles;
    garrison setVariable [_x,[],true];
    };
