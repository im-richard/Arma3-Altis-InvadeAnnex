...
if (_Pweapon != "") then
	{
	if ((_Pweapon == "hgun_PDW2000_F") or (_Pweapon == "rhs_weap_akms")) then
		{
		_necesita = true;
			...
			if (_unit distance _objeto < _distancia) then
				...
				if ((count weaponCargo _objeto > 0) and (isNil "_busy")) then
					...
					for "_i" from 0 to (count _armas - 1) do
						...
						if ((_posible != "hgun_PDW2000_F") and (_posible != "rhs_weap_akms") and ((_basePosible in arifles) or (_basePosible in srifles) or (_basePosible in mguns))) then
							{
							_target = _objeto;
							_hayCaja = true;
