...
_unit addEventHandler ["FIRED",
	...
	if (captive _unit) then
		{
		if ({((side _x== EAST) or (side _x== independent)) and (_x knowsAbout _unit > 1.4)} count allUnits > 0) then
			{
...
while {(captive player) and (captive _unit)} do
	...
	if ((_unit distance getMarkerPos _base < _size*2) and (_base in mrkAAF)) exitWith {_unit setCaptive true};
