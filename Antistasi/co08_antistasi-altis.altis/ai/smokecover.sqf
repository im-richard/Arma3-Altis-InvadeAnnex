...
while {sleep 1;({alive _x} count units _grupo > 0) and (canMove _veh) and (alive _veh)} do
	...
	if (_modo == "COMBAT") then
		...
		if (alive _veh) then
			...
			[_veh] call puertasLand;
			waitUntil {sleep 1; ({alive _x} count units _grupo == 0) or (not canMove _veh) or (not alive _veh) or (behaviour leader _grupo != "COMBAT")};
			{[_x] orderGetIn true; [_x] allowGetIn true} forEach _cargo;
			};
		};
	};
