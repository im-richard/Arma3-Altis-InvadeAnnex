...
_marcadores = marcadores + ["respawn_west"];
...
_chequeo = false;
{_enemigo = _x;
{if (((side _enemigo == east) or (side _enemigo == independent)) and (_enemigo distance _x < 500) and (not(captive _enemigo))) exitWith {_chequeo = true}} forEach units _grupo;
...
if (count _posicionTel > 0) then
	...
	{
		if (((side _x == east) or (side _x == independent)) and (_x distance (getMarkerPos _base) < 500) and (not(captive _x))) then {_chequeo = true};
	} forEach allUnits;
	...
	if (_posicionTel distance getMarkerPos _base < 50) then
		...
		if (!_esHC) then {cutText ["Fast traveling, please wait","BLACK",2]; sleep 2;} else {hcShowBar false;hcShowBar true;hint format ["Moving group %1 to destination",groupID _grupo]; sleep _distancia;};
		_forzado = false;
		...
		if (!_esHC) then {cutText ["You arrived to destination","BLACK IN",3]} else {hint format ["Group %1 arrived to destination",groupID _grupo]};
