...
[] call playerIsOwner;
_tsk = ["AtaqueAAF",true,[format ["CSAT is making a punishment expedition to %1. They will kill everybody there. Defend the city at all costs",_nombredest],"CSAT Punishment",_mrkDestino],getMarkerPos _mrkDestino,"CREATED",10,true,true,"Defend"] call BIS_fnc_setTask;
...
if ((({not (captive _x)} count _soldados) < ({captive _x} count _soldados)) or ({alive _x} count _soldados < round (_solMax / 3)) or (time > _tiempo)) then
	...
	[] call playerIsOwner;
	_tsk = ["AtaqueAAF",true,[format ["CSAT is making a punishment expedition to %1. They will kill everybody there. Defend the city at all costs",_nombredest],"CSAT Punishment",_mrkDestino],getMarkerPos _mrkDestino,"SUCCEEDED",10,true,true,"Defend"] call BIS_fnc_setTask;
	[-5,20,_posdestino] remoteExecCall ["citySupportChange",2];
	[10,0] remoteExecCall ["prestige",2];
	{[-5,0,_x] remoteExecCall ["citySupportChange",2]} forEach ciudades;
	...
	{
	[] call playerIsOwner;
	_tsk = ["AtaqueAAF",true,[format ["CSAT is making a punishment expedition to %1. They will kill everybody there. Defend the city at all costs",_nombredest],"CSAT Punishment",_mrkDestino],getMarkerPos _mrkDestino,"FAILED",10,true,true,"Defend"] call BIS_fnc_setTask;
	[-5,-20,_posdestino] remoteExecCall ["citySupportChange",2];
	{[0,-5,_x] remoteExecCall ["citySupportChange",2]} forEach ciudades;
