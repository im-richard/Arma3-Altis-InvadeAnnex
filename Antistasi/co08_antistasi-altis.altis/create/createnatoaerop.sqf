...
if (count _buildings > 1) then
	...
	while {(spawner getVariable _marcador) and (_cuenta < round (5*_prestigio))} do
		{
		_tipoveh = planesNATO call BIS_fnc_selectRandom;
...
_grupo = [_posicion, west, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad")] call BIS_Fnc_spawnGroup;
sleep 1;
...
while {(spawner getVariable _marcador) and (_cuenta < _nVeh)} do
	...
	if (diag_fps > minimoFPS) then
		...
		_grupo = [_pos,west, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad")] call BIS_Fnc_spawnGroup;
		sleep 1;
...
if (("b_g_soldier_unarmed_f" in _garrison) or ({typeOf _x == "B_G_Mortar_01_F"} count _estaticas > 0)) then
	{
	_grupoMort = createGroup WEST;
	};
_grupoEst = grpNull;
if (count _estaticas > 0) then
	{
	_grupoEst = createGroup WEST;
	};
_grupoMed = grpNull;
...
while {(spawner getVariable _marcador) and (_cuenta < _tam)} do
	...
	if (_tipo == "b_g_soldier_unarmed_f") then
		...
		if ((_tipo == "B_G_Soldier_F") and (count _estaticas > 0)) then
			...
			{
			if ((_tipo == "B_G_medic_F") and (isNull _grupoMed)) then
				{
				_grupoMed = createGroup WEST;
				_unit = _grupoMed createUnit [_tipo, _posicion, [], 0, "NONE"];
				_wp = _grupoMed addWaypoint [_posicion,5];
...
if (!alive _soldado) then
	...
	for "_i" from 0 to (count _garrison -1) do
		{
		if (typeOf _soldado == (_garrison select _i)) exitWith {_garrison deleteAt _i};
		};
	}
else
	{
...
if (_marcador in mrkFIA) then {garrison setVariable [_marcador,_garrison,true]};
if (!isNull _periodista) then {deleteVehicle _periodista};
{deleteGroup _x} forEach _gruposFIA;
{if (alive _x) then {deleteVehicle _x}} forEach _soldados;
{deleteGroup _x} forEach _grupos;
deleteGroup _grupoEst;
deleteGroup _grupoMed;
