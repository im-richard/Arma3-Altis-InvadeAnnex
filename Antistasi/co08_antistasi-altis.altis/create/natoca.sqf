...
[] call playerIsOwner;
_tsk = ["NATOCA",true,[format ["Our Commander asked NATO for an attack on %1. Help them in order to have success in this operation. Their attack will depart from %2",_nombredest,_nombreorig],"NATO Attack",_marcador],_posicion,"CREATED",5,true,true,"Attack"] call BIS_fnc_setTask;
_soldados = [];
_vehiculos = [];
_grupos = [];
_tipoveh = "";
...
for "_i" from 1 to _cuenta do
	...
	_vehicle=[_orig, 0, _tipoveh, WEST] call bis_fnc_spawnvehicle;
	...
	*/
	...
	if (_tipoveh == "B_Heli_Transport_01_camo_F") then
		{
		_grupo = [_orig, WEST, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad_Weapons")] call BIS_Fnc_spawnGroup;
	...
	if (_tipoveh == "B_Heli_Light_01_F") then
		{
		{_x disableAI "TARGET"; _x disableAI "AUTOTARGET"} foreach units _grupoheli;
		_grupo = [_orig, WEST, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfTeam")] call BIS_Fnc_spawnGroup;
		{_x assignAsCargo _heli; _x moveInCargo _heli; _soldados = _soldados + [_x]; [_x] spawn NATOinitCA} forEach units _grupo;
		_grupos = _grupos + [_grupo];
		_landpos = [];
		_landpos = [_posicion, 0, 500, 10, 0, 0.3, 0] call BIS_Fnc_findSafePos;
	...
	if (_tipoveh == "B_Heli_Transport_03_F") then
		{
		{_x disableAI "TARGET"; _x disableAI "AUTOTARGET"} foreach units _grupoheli;
		_grupo = [_orig, WEST, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfAssault")] call BIS_Fnc_spawnGroup;
		{_x assignAsCargo _heli; _x moveInCargo _heli; _soldados = _soldados + [_x]; [_x] spawn NATOinitCA} forEach units _grupo;
		_grupos = _grupos + [_grupo];
...
if ({alive _x} count _soldados < _solMax) then
	{
	[] call playerIsOwner;
	_tsk = ["NATOCA",true,[format ["Our Commander asked NATO for an attack on %1. Help them in order to have success in this operation. Their attack will depart from %2",_nombredest,_nombreorig],"NATO Attack",_marcador],_posicion,"FAILED",5,true,true,"Attack"] call BIS_fnc_setTask;
