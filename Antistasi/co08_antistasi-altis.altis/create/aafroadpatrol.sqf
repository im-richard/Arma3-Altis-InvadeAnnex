...
while {true} do
	...
	if (_tipoCoche == "I_Heli_light_03_unarmed_F") then
		{
...
_vehicle=[_posbase, 0,_tipoCoche, independent] call bis_fnc_spawnvehicle;
...
if (_tipoCoche == "I_MRAP_03_F") then
	{
	sleep 1;
	_grupo = [_posbase, independent, (configfile >> "CfgGroups" >> "Indep" >> "IND_F" >> "Infantry" >> "HAF_InfSentry")] call BIS_Fnc_spawnGroup;
...
while {alive _veh} do
	...
	if (debug) then {player sidechat format ["Patrulla AAF generada. Origen: %2 Destino %1", _destino, _base]; sleep 3;};
