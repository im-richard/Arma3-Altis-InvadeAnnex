...
if (_marcador in mrkAAF) then
	...
	_params = [_posicion, independent, (configfile >> "CfgGroups" >> "Indep" >> "IND_F" >> "Infantry" >> "HAF_InfSentry")];
	}
else
	{
	_esAAF = false;
	_num = round (_num * (_prestigeBLUFOR/100));
	_params = [_posicion, west, (configfile >> "CfgGroups" >> "West" >> "Guerilla" >> "Infantry" >> "IRG_InfSentry")];
	};

if (_num < 1) then {_num = 1};

if (debug) then {stavros sideChat format ["Voy a crear %1 grupos en %2", _num,_marcador]; sleep 2};
...
if (_esAAF) then
	{
	{_grp = _x;
	{[_x] spawn AAFinitBASES; _soldados = _soldados + [_x]} forEach units _grp;} forEach _grupos;
	}
else
	{
	{_grp = _x;
