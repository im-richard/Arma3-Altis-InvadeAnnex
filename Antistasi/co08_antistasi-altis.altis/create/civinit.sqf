...
_EHkilledIdx = _unit addEventHandler ["killed",
	...
	if (_muerto == _killer) then
		{
		_nul = [-1,-1,getPos _muerto] remoteExecCall ["citySupportChange",2];
		}
	else
		{
		if (isPlayer _killer) then
			...
			if (!isMultiPlayer) then
				{
				_nul = [0,20] remoteExecCall ["resourcesFIA",2];
		...
		if (side _killer == WEST) then
			{
			_nul = [-1*_multiplicador,0] remoteExecCall ["prestige",2];
			...
			if (side _killer == independent) then
				...
				if (side _killer == east) then
					{
					_nul = [2*_multiplicador,0] remoteExecCall ["prestige",2];
					_nul = [-1,1,getPos _muerto] remoteExecCall ["citySupportChange",2];
					};
				};
			};
