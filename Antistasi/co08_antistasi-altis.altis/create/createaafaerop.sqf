...
while {(spawner getVariable _marcador) and (_cuenta < 4)} do
	...
	_grupos = _grupos + [_grupo];
	{[_x] spawn AAFinitBASES; _soldados pushBack _x} forEach units _grupo;
	_cuenta = _cuenta +1;
	};
...
if (!_busy) then
	...
	if (count _buildings > 1) then
		...
		while {(spawner getVariable _marcador) and (_cuenta < 5)} do
			...
			_unit = _grupo createUnit ["I_pilot_F", _posicion, [], 0, "NONE"];
			[_unit] spawn AAFinitBASES;
			_cuenta = _cuenta + 1;
			};
...
//waitUntil {sleep 1; (not (spawner getVariable _marcador)) or (({(alive _x) and (_x distance _posicion < _size) and (not(vehicle _x isKindOf "Air"))} count amigos) + ({(alive _x) and (_x distance _posicion < _size) and (not(vehicle _x isKindOf "Air"))} count NATO) > 3*({(alive _x) and (!(captive _x)) and (_x distance _posicion < _size)} count _soldados))};
